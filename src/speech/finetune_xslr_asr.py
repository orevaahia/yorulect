...
os.environ["WANDB_PROJECT"] = "yoruba_dialects"
...
def create_vocabulary_from_data(
    ...
    vocab_set = functools.reduce(
        lambda acc, vocab: acc | set(vocab["vocab"][0]),
        vocabs.values(),
    set()
    )
...
def main():
    ...
    if training_args.do_train:
        raw_datasets["train"] = load_dataset("audiofolder",
        ...
        if data_args.audio_column_name not in raw_datasets["train"].column_names:
            raise ValueError(
                f"--audio_column_name '{data_args.audio_column_name}' not found in dataset '{data_args.dataset_dir}'."
        ...
        if data_args.text_column_name not in raw_datasets["train"].column_names:
            raise ValueError(
                f"--text_column_name {data_args.text_column_name} not found in dataset '{data_args.dataset_dir}'. "
    ...
    if training_args.do_eval:
        raw_datasets["eval"] = load_dataset("audiofolder",
            data_dir=data_args.dataset_dir,
    ...
    if training_args.do_predict:
        raw_datasets["test"] = load_dataset("audiofolder",
            data_dir=data_args.dataset_dir,
            cache_dir=model_args.cache_dir,
            split=data_args.test_split_name,
        )
        if data_args.max_test_samples is not None:
                raw_datasets["test"] = raw_datasets["test"].select(range(data_args.max_test_samples))
    ...
    basename = os.path.basename(training_args.output_dir)

    # Define evaluation metrics during training, *i.e.* word error rate, character error rate
    eval_metrics = {metric: evaluate.load(metric, cache_dir=model_args.cache_dir, experiment_id=f"{basename}wer") for metric in data_args.eval_metrics}
    ...
    if training_args.do_predict:
        ...
        max_test_samples = (
            data_args.max_test_samples if data_args.max_test_samples is not None else len(vectorized_datasets["test"])
        )
        test_metrics["test_samples"] = min(max_test_samples, len(vectorized_datasets["test"]))
    ...
    kwargs = {
        ...
        "tags": ["automatic-speech-recognition", data_args.dataset_dir],
        "dataset_args": (
            f"Config: {config_name}, Training split: {data_args.train_split_name}, Eval split:"
            f" {data_args.eval_split_name}"
        ),
        "dataset": f"{data_args.dataset_dir.upper()} - {config_name.upper()}",
    }
    if "common_voice" in data_args.dataset_dir:
